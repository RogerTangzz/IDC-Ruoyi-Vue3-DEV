...
const templateSelector = {
  ...
  selectTemplate(requirement) {
    ...
    const keywords = {
      '列表|list|index': 'list',
      '表单|form|edit|create': 'form',
      '详情|detail|view': 'detail',
      'api|接口|crud': 'crud',
      'store|状态|pinia': 'store'
    };
  ...
  // 生成代码
  generate(requirement, variables = {}) {
    const templateName = this.selectTemplate(requirement);
  ...
  loadTemplate(name) {
    // 这里返回对应的模板内容
    return templates[name] || templates.base;
  },
...
```javascript
// 代码生成前的上下文分析
export const contextAwareGeneration = {
  // 1. 代码风格分析
  async analyzeCodeStyle(projectPath) {
    const analysis = {
      naming: await this.detectNamingConvention(projectPath),
      imports: await this.analyzeImportStyle(projectPath),
  ...
  // 2. 命名规范检测
  async detectNamingConvention(path) {
    // 分析现有文件的命名模式
    const patterns = {
      component: 'PascalCase',  // MyComponent.vue
      composable: 'camelCase',   // useMyHook.js
      constant: 'UPPER_SNAKE',   // MY_CONSTANT
      file: 'kebab-case'        // my-file.js
    };
  ...
  // 3. 智能导入管理
  async manageImports(code, context) {
    const imports = {
      // Vue 3 自动导入
      ...
      // 项目内部导入
      '@/utils': ['request', 'auth', 'cache'],
      '@/hooks': context.hooks || []
    };
    
    return this.optimizeImports(code, imports);
  },

  // 4. 类型推导
  async inferTypes(apiSchema, usage) {
    const types = {
      request: this.inferRequestTypes(apiSchema),
      response: this.inferResponseTypes(apiSchema),
      props: this.inferPropsFromUsage(usage)
    };
    
    return this.generateTypeDefinitions(types);
  },

  // 5. 错误模式识别
  detectErrorPatterns(code) {
    const patterns = [
      { pattern: /console\.log/, severity: 'warning', message: '生产环境应移除console.log' },
      { pattern: /any/, severity: 'warning', message: '避免使用any类型' },
      { pattern: /\/\/\s*TODO/, severity: 'info', message: '存在待办事项' }
    ];
...
// 使用示例
const context = await contextAwareGeneration.analyzeCodeStyle('./src');
const code = templateSelector.generate('创建用户列表页面', {
  module: 'user',
  domain: 'system'
});
const optimizedCode = await contextAwareGeneration.manageImports(code, context);
```
## 文件级回滚
...
export default {
  ...
  loadOrder: [
    'CLAUDE.md',           // 核心规范
    '.clauderc.js',        // 项目配置
    'CLAUDE-{PROJECT}.md'  // 项目扩展
  ],

  // 2. 配置合并策略
  mergeStrategy: {
    rules: 'override',      // 规则覆盖
    templates: 'extend',    // 模板扩展
    patterns: 'merge'       // 模式合并
  },

  // 3. AI助手行为配置
  aiAssistant: {
    autoDetect: true,       // 自动检测项目类型
    contextAware: true,     // 上下文感知
    codeStyle: 'project'    // 遵循项目风格
  }
}

// 配置优先级：项目配置 > 扩展文档 > 核心文档
```
